...
4. `arr->mean(arr)` finds the mean of the result.
...
```
oly = load("../code/datasets/OlympicAthletes_0.csv") |> DataFrame;
```
...
```
oly |> @filter(_.Year == 2000) |> @orderby_descending(_.Total_Medals) |> DataFrame |> head
```
...
```
df |> @groupby(_.name) |> @map({Name=_.name,Num_cars=length(_.car)}) |> DataFrame
```
...
oly |>
  @groupby(_.Athlete) |>
  @map({Athlete = unique(_.Athlete)[1], total = sum(_.Total_Medals)}) |>
...
us_medals_by_year = oly |>
  ...
  @groupby(_.Year) |>
  @map({Year = unique(_.Year)[1], Total_Medals = sum(_.Total_Medals)})  |>
  @orderby(_.Year) |> DataFrame
```
