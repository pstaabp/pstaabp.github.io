...
where $\Delta x=(b-a)/N$ and $x_i^{\star}$ is a number in the $i$th subinterval.  Generally, $x_i^{\star}$ can be the left endpoint, the right end point, the mid point or a value at the minimum or maximum of the function in the subinterval. 
