...
Find all relative extrema of $g(x)=\left|x^{2}-4\right|$.
...
Find all relative extrema of $(x^{3}-4x)^{1/3}$.  First, find the extrema using the definition and determine if each is either a relative minimum, maximum or neither.  You can verify the results with a plot at the end.
...
1. The smallest function value is 0, so this is the absolute minimumum and the largest function value is 12, so this is the absolute maximum.  The absolute minimum occurs at $x=2$ and the absolute maximum occurs at $x=4$.
...
### Example: built-in function to find a Maximum
