...
Alternatively, prime numbers have only 2 numbers in factor list and so it can be checked if the length of the factors is 2. 
...
* Stop the code if you need to. To stop any code running in Matlab, enter CTRL-C (that is, hold the control key, then the `C` character). 
...
In addition, the `~` operator switches a true to false and a false to true. A simple version of this would be if we had an `isOdd` function as written above, then the following would be an `isEven` function:
