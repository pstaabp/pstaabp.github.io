...
Recall that if we have a function $y=f(x)$ on the interval $a \leq x \leq b$, then the arc length of the curve on this interval is
...
where $r=x$ if rotating about the $y$-axis and $x=f(x)$ if rotating about the $x$-axis.  
