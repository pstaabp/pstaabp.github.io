...
$$ f'(x) = \lim_{x \rightarrow \infty} \frac{f(x+h)-f(x)}{h} $$

(Note: formatting troubles if we put in $f(x)=x^2$)
...
```
implicitplot([x^2+y^2=25,y=-3/4(x-3)+4],x=-6..6,y=-6..6)
```

![Plot of a circle of radius 5 and its tangent line](images/ch04/plot10.png)

where the `color` option has been used to distinguish between the curves and a legend has been added.  
