...
```
createZ := proc(xRange::range, yRange::range, nx::posint, ny::posint)::Matrix;
  local dx, dy;
  dx := (rhs(xRange)-lhs(xRange))/(nx-1);
  dy := (rhs(yRange)-lhs(yRange))/(ny-1);
  Matrix(nx, ny, (i, j)-> lhs(xRange)+dx*(j-1)+I*rhs(yRange)-I*dy*(i-1));
end proc
```
...
NewtonPlot := proc (xRange::range, yRange::range, nx::posint, ny::posint)
  ...
  Z := createZ(xRange, yRange, nx, ny);
  zeros := closest(Z, 0+0*I);
  Z[op(zeros[1])] := 0.1e-4*I+0.1e-4;
  roots := map(a->newton(z->z^3-1, a), Z);
  angles := map(argument, roots);
  plots[listdensityplot](LinearAlgebra[Transpose](angles), range = -Pi .. Pi, colorstyle = HUE, style = patchnogrid, axes = none, smooth = true):
end proc
```
