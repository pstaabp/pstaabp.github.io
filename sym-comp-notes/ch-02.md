...
solve(4x+3>5x-7,x)
```
will return *RealRange(-$\infty$,Open(10))*.  This is Maple's notation for $(-\infty,10)$.  Try changing the greater than sign in the inequality to a greater than or equal to, which you type in `>=` and Maple will change it automatically to a $\geq$.  
...
1. Enter[^lists]:`dots:=[[0,1],[1,0],[2,1],[3,0]]`
2. Enter: `plot(dots,style=point)`
...
1200-984\,{ {\rm e}^{x} }+729\,{ {\rm e}^{2\,x} } } } } }\\]
 ...
 984\,{ {\rm e}^{x}}+729\,{ {\rm e}^{2\,x} } }$ is defined, so solve:
    ```
    solve(1200-984\,{ {\rm e}^{x} }+729\,{ {\rm e}^{2\,x} } \geq 0)
    ```
